...
> **Prompt-architect** ships with extra reference files (`references/domains.md` and `references/techniques.md`). Download the whole `prompt-architect/` folder, not just `SKILL.md`, and keep the `references/` subfolder next to it so the skill can read them.
